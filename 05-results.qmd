...
#library(websocket) # For handling web socket communication
...
#cleaned_data$displacement_times <- as.numeric(cleaned_data$displacement_times)
...
labels <- sapply(cleaned_data, label) # Extract labels from cleaned data
 ...
 # Ensure labels is a named list with variable names as names
  #labels <- setNames(as.list(labels), label)
...
create_summary_table <- function(data, by_var, vars, labels, caption_prefix) {
  ...
  gt_table <- as_gt(summary_table)%>%
    cols_width(
      everything() ~ px(150)  # Adjust column width as needed
    )
...
#| fig-cap: Income Distribution Before and After Conflict \n Among IDPs & Host Community
...
violin_plot <- ggplot(data_long, 
  ...
  theme_grey() +
  labs(#title = "Income Distribution Before and After Conflict \n Among IDPs & Host Community",
...
### Shared Values & Goals Between IDPs & Host Community
...
#| tbl-cap: Social Integration & Participation of IDPs Towards Host Community
...
gt_table <- cleaned_data %>%
  ...
  modify_header(label ~ "**Variable**") %>% # Modify the header for variable names
    bold_labels() %>%
    modify_caption("**Social Integration & Participation of IDPs Towards Host Community**") # Add caption to the table
...
#| tbl-cap: Shared Values & Goals Information Stratified by Displacement status
...
gt_table %>%
  gt_theme_538()  # Example of applying a theme from gtExtra
...
#| tbl-cap: Accessibility Factors to Basic Resources Between IDPs & Host Community
...
-   Overall, most respondents indicated "No role" (80%), followed by "Supportive" (15%) and "Opposing" (4.1%).

-   Among host community members and IDPs, there were similar proportions across these roles with no significant differences observed (p = 0.2).

**Role of Religious Leaders:**

-   A significant majority reported support from religious leaders (60%), with fewer indicating "No role" (40%) and none opposing.

-   Similar proportions were observed among host community members (63% support) and IDPs (59% support), with no significant differences (p \> 0.05).

**Role of Tribal Sheiks:**

-   Responses were divided fairly evenly between "Supportive" (47%) and "No role" (51%), with a small percentage (1.6%) opposing.

-   Host community members and IDPs showed similar distributions across these roles, with no significant differences (p \> 0.9).

**Support from NGOs:**

-   The majority reported "No role" from NGOs (72%), while 27% indicated support, and a very small percentage opposed (0.5%).

-   Distribution among host community members (71% no role) and IDPs (73% no role) did not differ significantly (p = 0.8).

**Role of Youth Engagement:**

-   Most respondents were supportive of youth engagement (61%), followed by "No role" (37%) and a small percentage (2.0%) opposing.

-   Host community members (67% supportive) and IDPs (57% supportive) differed slightly, approaching significance (p = 0.062).

**Community Involvement (People Themselves):**

-   A significant majority reported community involvement (70% supportive), with fewer indicating "No role" (26%) and a small percentage (4.4%) opposing.

-   Distribution among host community members (74% supportive) and IDPs (67% supportive) did not show significant differences (p = 0.3).
...
#### Associations Roles of Governments & other bodies in Relation to Percieved Social Cohesion
...
1.  **Role of Government Institutions and Authorities:**
    ...
    -   The perception of the role of government institutions is significantly associated with the level of social cohesion among respondents (p = 0.008). Specifically:

    -   Respondents who perceive government institutions as supportive are more likely to report low (19%) and acceptable (17%) cohesion compared to high cohesion (11%).

2.  **Role of Religious Leaders:**

    -   Among those reporting low social cohesion, 44% perceived no role from religious leaders, compared to 38% in the acceptable cohesion group and 41% in the high cohesion group.

    -   Supportive roles from religious leaders showed a significant association (p = 0.017) with higher social cohesion levels.

3.  **Role of Tribal Sheiks:**

    -   For those with low social cohesion, 60% reported no role from tribal sheiks, compared to 44% in the acceptable cohesion group and 65% in the high cohesion group.

    -   Supportive roles from tribal sheiks were significantly associated (p = 0.017) with higher social cohesion.
...
5.  **Role of Youth Engagement:**
    ...
    -   Opposition to youth engagement was higher among those with low social cohesion (8.3%) compared to 1.6% in the acceptable cohesion group and none in the high cohesion group.

    -   Supportive roles from youth engagement were strongly associated (p \< 0.001) with higher social cohesion levels.

6.  **Community Involvement (People Themselves):**

    -   Those reporting low social cohesion had the highest proportion (21%) indicating opposition to community involvement, compared to 3.2% in the acceptable cohesion group and none in the high cohesion group.

    -   Strong support for community involvement showed a significant association (p \< 0.001) with higher social cohesion.
...
#| tbl-cap: Associations Between the Levels of Shared Values & Goals and Various Variables 
...
**Gender**: Gender was not significantly associated with resilience levels (p = 0.3). Among those with low resilience, 60% were female and 40% were male. In the average resilience group, 64% were female and 36% were male, and for high resilience, 72% were female and 28% were male.
...
#### Roles of Governments & other bodies in Relation with Resilience
...
5.  **Role of Youth Engagement:** There was no significant association (p = 0.2) between roles of youth engagement and resilience scores. However, supportive roles were slightly higher among those with high resilience scores (69%).
...
**Health Status over the Last 6 Months:** Overall, respondents reported their health status as "Fairly good" (43%) most frequently, followed by "Good" (36%) and "Bad" (21%). There were slight differences between host community members (Bad: 17%, Fairly good: 45%, Good: 38%) and IDPs (Bad: 24%, Fairly good: 42%, Good: 34%), with no significant overall difference (p = 0.3).

**Physical Health Status:** Responses were distributed fairly evenly across the scale from 1 (poor) to 5 (excellent), with no significant differences between host community members and IDPs (p = 0.4).

**Relationships with Family, Including Spouse/Partner:** A significant majority reported positive relationships (score of 5), but there were notable differences. Host community members (67%) tended to rate their relationships higher than IDPs (45%) (p \< 0.001).

**Relationships with Friends:** Similar to family relationships, a majority reported positive friendships (score of 5), with host community members (43%) more likely than IDPs (22%) to rate relationships at this level (p \< 0.001).

**Worry about Global Events:** Respondents expressed varying levels of worry, with a majority leaning towards higher levels of concern (score of 5). IDPs (42%) were slightly more likely than host community members (38%) to express higher worry levels, but the difference was not significant (p = 0.038).

**Optimism about the Next 12 Months:** Overall, optimism varied, with a majority indicating moderate to high levels of optimism (scores of 4 and 5). Host community members (58%) tended to be more optimistic compared to IDPs (42%) (p = 0.027).
...
The associations between various wellbeing attributes and perceived levels of social cohesion are summarized in @tbl-health-cohesion & @tbl-health-cohesion-resilience. Below are the detailed findings for each attribute:
...
**Relationships with Friends:** This attribute showed a significant association with social cohesion (p = 0.002). In the low cohesion group, 13% rated their relationships with friends as 1 (worst), 19% as 2, 29% as 3, 17% as 4, and 23% as 5 (best). For acceptable cohesion, the ratings were 7.7%, 19%, 29%, 15%, and 30%. In the high cohesion group, the ratings were 4.4%, 9.6%, 22%, 33%, and 31%.

**Worry about Global Events:** There was no significant association (p = 0.079).

**Optimism about the Next 12 Months:** This attribute was significantly associated with social cohesion (p = 0.047). In the low cohesion group, 27% rated their optimism as 1 (least optimistic), 19% as 2, 17% as 3, 6.3% as 4, and 31% as 5 (most optimistic). For acceptable cohesion, the ratings were 17%, 21%, 21%, 15%, and 26%. In the high cohesion group, the ratings were 15%, 13%, 15%, 17%, and 40%.
...
**Health Status (last 6 months):** There was a significant association between health status and levels of resilience (p \< 0.001). Among respondents with low resilience, 37% reported having bad health, 41% fairly good health, and 22% good health. Those with average resilience had 19% reporting bad health, 42% fairly good health, and 40% good health. For high resilience, 11% reported bad health, 51% fairly good health, and 38% good health.

**Physical Health Status:** This attribute also showed a significant association with resilience levels (p \< 0.001). For low resilience, 18% rated their physical health as 1 (worst), 19% as 2, 31% as 3, 22% as 4, and 10% as 5 (best). In the average resilience group, the ratings were 3.9%, 10%, 42%, 27%, and 17%. For high resilience, the ratings were 0%, 6.2%, 37%, 38%, and 18%.

**Relationships with Family:** Significant associations were found between family relationships and resilience levels (p = 0.004). In the low resilience group, 5.6% rated their family relationships as 1 (worst), 7.8% as 2, 14% as 3, 32% as 4, and 40% as 5 (best). For average resilience, the ratings were 2.0%, 4.7%, 18%, 22%, and 54%. In the high resilience group, the ratings were 0%, 0%, 7.7%, 23%, and 69%.

**Relationships with Friends:** This attribute showed a significant association with resilience levels (p \< 0.001). For low resilience, 10% rated their relationships with friends as 1 (worst), 36% as 2, 23% as 3, 14% as 4, and 17% as 5 (best). In the average resilience group, the ratings were 7.8%, 11%, 29%, 21%, and 31%. In the high resilience group, the ratings were 1.5%, 9.2%, 23%, 23%, and 43%.

**Worry about Global Events:** There was a significant association between worry about global events and resilience levels (p \< 0.001). For low resilience, 8.9% rated their worry about global events as 1 (least worried), 1.1% as 2, 22% as 3, 16% as 4, and 52% as 5 (most worried). In the average resilience group, the ratings were 10%, 3.9%, 28%, 20%, and 38%. For high resilience, the ratings were 7.7%, 17%, 32%, 17%, and 26%.

**Optimism about the Next 12 Months:** This attribute was significantly associated with resilience levels (p \< 0.001). In the low resilience group, 37% rated their optimism as 1 (least optimistic), 24% as 2, 7.8% as 3, 11% as 4, and 20% as 5 (most optimistic). For average resilience, the ratings were 13%, 20%, 21%, 15%, and 31%. In the high resilience group, the ratings were 6.2%, 7.7%, 25%, 18%, and 43%.
